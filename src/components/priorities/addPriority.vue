  ...
  <v-row>
    <v-col class="col-6">
      <label class="regularFont login-font" style=""
        >Event <small style="color: red">*</small></label
      >
      ...
      <div class="invalid-feedback" v-if="$v.priorityModel.title.$error">
        <span v-if="$v.priorityModel.title.$error">Event is required</span>
      </div>
    </v-col>
    <v-col class="col-6">
      <label class="regularFont login-font" style=""
    ...
    <v-col class="col-6">
      ...
      >
      <v-radio-group v-model="priorityModel.status" row>
  ...
  methods: {
    ...
    submit() {
      ...
      if (!this.$v.$invalid) {
        if (!this.isEdit) {
          this.$store
            .dispatch("addPriority", this.priorityModel)
            .then((res) => {
              if (res.success) {
                Swal.fire({
                  icon: "success",
          ...
          this.$store
            .dispatch("editPriority", this.priorityModel)
            .then((res) => {
              if (res.success) {
                Swal.fire({
                  icon: "success",
