  ...
  <div>
    ...
    <div class="tableWrapper">
      <table class="row mx-0" id="myTable">
        ...
        <tbody class="px-0">
          <tr v-for="(data, index) in getroleList" :key="index" class="px-0 tabledata">
            <td class="name">{{data._id}}</td>
<td class="email">{{data.createdAt}}</td>
...
export default {
  ...
  computed: {
    ...mapGetters(["getuserList","getwhiteList","getroleList", "userDetails"]),
    maxPages() {
     return Math.ceil(this.getwhiteList / 10)
    }
  },
};
</script>
